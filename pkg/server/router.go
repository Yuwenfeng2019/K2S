...
func router(serverConfig *config.Control, tunnel http.Handler, ca []byte) http.Handler {
	...
	authed.Path("/v1-k2s/serving-kubelet.crt").Handler(servingKubeletCert(serverConfig))
	authed.Path("/v1-k2s/serving-kubelet.key").Handler(fileHandler(serverConfig.Runtime.ServingKubeletKey))
	authed.Path("/v1-k2s/client-kubelet.crt").Handler(clientKubeletCert(serverConfig))
	authed.Path("/v1-k2s/client-kubelet.key").Handler(fileHandler(serverConfig.Runtime.ClientKubeletKey))
	authed.Path("/v1-k2s/client-kube-proxy.crt").Handler(fileHandler(serverConfig.Runtime.ClientKubeProxyCert))
	authed.Path("/v1-k2s/client-kube-proxy.key").Handler(fileHandler(serverConfig.Runtime.ClientKubeProxyKey))
	authed.Path("/v1-k2s/client-k2s-controller.crt").Handler(fileHandler(serverConfig.Runtime.ClientK2sControllerCert))
	authed.Path("/v1-k2s/client-k2s-controller.key").Handler(fileHandler(serverConfig.Runtime.ClientK2sControllerKey))
