...
func NewServerCommand(action func(*cli.Context) error) cli.Command {
	return cli.Command{
		...
		Flags: []cli.Flag{
			...
			cli.StringFlag{
				Name:        "bind-address",
				Usage:       "(listener) k2s bind address (default: 0.0.0.0)",
			...
			cli.StringFlag{
				Name:        "data-dir,d",
				Usage:       "(data) Folder to hold state default /var/lib/yuwenfeng/k2s or ${HOME}/.rancher/k2s if not root",
			...
			cli.StringFlag{
				...
				EnvVar:      "K2S_CLUSTER_SECRET,K2S_TOKEN",
			},
			cli.StringFlag{
				Name:        "token-file",
			...
			cli.BoolFlag{
				...
				Usage:       "(components) Disable k2s default cloud controller manager",
				Destination: &ServerConfig.DisableCCM,
			},
			cli.BoolFlag{
				Name:        "disable-network-policy",
				Usage:       "(components) Disable k2s default network policy controller",
