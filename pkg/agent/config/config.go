...
func getNodeNamedCrt(nodeName, nodePasswordFile string) HTTPRequester {
	return func(u string, client *http.Client, username, password string) ([]byte, error) {
		...
		req.Header.Set("K2s-Node-Name", nodeName)
		nodePassword, err := ensureNodePassword(nodePasswordFile)
		if err != nil {
			return nil, err
		}
		req.Header.Set("K2s-Node-Password", nodePassword)
...
func getServingCert(nodeName, servingCertFile, servingKeyFile, nodePasswordFile string, info *clientaccess.Info) (*tls.Certificate, error) {
	servingCert, err := Request("/v1-k2s/serving-kubelet.crt", info, getNodeNamedCrt(nodeName, nodePasswordFile))
	if err != nil {
		return nil, err
	}
...
func getHostFile(filename, keyFile string, info *clientaccess.Info) error {
	...
	fileBytes, err := clientaccess.Get("/v1-k2s/"+basename, info)
	if err != nil {
		return err
	}
	if keyFile == "" {
...
func getNodeNamedHostFile(filename, keyFile, nodeName, nodePasswordFile string, info *clientaccess.Info) error {
	basename := filepath.Base(filename)
	fileBytes, err := Request("/v1-k2s/"+basename, info, getNodeNamedCrt(nodeName, nodePasswordFile))
	if err != nil {
		return err
	}
	fileBytes, keyBytes := splitCertKeyPEM(fileBytes)
...
func locateOrGenerateResolvConf(envInfo *cmds.Agent) string {
	...
	tmpConf := filepath.Join(os.TempDir(), "k2s-resolv.conf")
...
func get(envInfo *cmds.Agent) (*config.Node, error) {
	...
	clientK2sControllerCert := filepath.Join(envInfo.DataDir, "client-k2s-controller.crt")
	clientK2sControllerKey := filepath.Join(envInfo.DataDir, "client-k2s-controller.key")
	if err := getHostFile(clientK2sControllerCert, clientK2sControllerKey, info); err != nil {
		return nil, err
	}

	kubeconfigK2sController := filepath.Join(envInfo.DataDir, "k2scontroller.kubeconfig")
	if err := control.KubeConfig(kubeconfigK2sController, info.URL, serverCAFile, clientK2sControllerCert, clientK2sControllerKey); err != nil {
		return nil, err
	}
	...
	nodeConfig.AgentConfig.KubeConfigK2sController = kubeconfigK2sController
	if envInfo.Rootless {
	...
	nodeConfig.Containerd.State = "/run/k2s/containerd"
...
func getConfig(info *clientaccess.Info) (*config.Control, error) {
	data, err := clientaccess.Get("/v1-k2s/config", info)
	if err != nil {
		return nil, err
	}
